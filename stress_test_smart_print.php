...
test("PrintSession model works", function() {
    $session = PrintSession::create([
        ...
        'barcode_token' => 'TEST_BARCODE_' . uniqid(),
...
test("Generate new print session", function() use (&$testSession) {
    ...
    $testSession = $session;
    return $session && $session->session_token ? true : "Failed to generate session";
});
...
test("Calculate print price", function() use ($testSession, $selectedVariant, &$calculatedPrice) {
    ...
    return isset($result['total_price']) && $result['total_price'] > 0 ? true : 'Price calculation failed';
});
...
test("Order has correct status", function() use ($testOrder) {
    ...
    return in_array($testOrder->status, ['pending', 'pending_upload', 'uploaded']) ? true : "Order status is: " . $testOrder->status;
});
...
test("Expired session handling", function() {
    $expiredSession = PrintSession::create([
        ...
        'barcode_token' => 'EXPIRED_BARCODE_' . uniqid(),
